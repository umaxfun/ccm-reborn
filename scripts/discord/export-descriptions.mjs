...
const SIDEBAR_GUILD_ID = "967712302767960064";
...
const SIDEBAR_NON_MOD_CHANNEL = /(^|-)bug-report(-|$)|patch-notes|(^|-)news(-|$)|troubleshooting|discussion|feedback|^releases-and-news$|notification-self-assign|ccm-development|(^|-)instructions$|^search-|synergys-custom-campaign-launcher|getting-started|faq/i;
...
function usage() {
  ...
  npm run discord:export -- --from-sidebar [--output <path>]
...
async function readInventorySidebarModChannels() {
  const inventory = JSON.parse(await readFile(INVENTORY_PATH, "utf8"));
  if (inventory.format !== 1 || !Array.isArray(inventory?.guilds)) {
    throw new Error(`${INVENTORY_PATH} is not a Discord RPC inventory generated by this script.`);
  }
  const configuredGuildId = process.env.DISCORD_GUILD_ID?.trim() ?? SIDEBAR_GUILD_ID;
  const guilds = configuredGuildId
    ? inventory.guilds.filter((guild) => guild.id === configuredGuildId)
    : inventory.guilds;
  if (configuredGuildId && guilds.length === 0) {
    throw new Error(`${INVENTORY_PATH} does not contain DISCORD_GUILD_ID=${configuredGuildId}. Refresh the inventory first.`);
  }
  return guilds
    .flatMap((guild) => guild.channels ?? [])
    .filter((channel) => channel.type === 0 && typeof channel.id === "string" && !SIDEBAR_NON_MOD_CHANNEL.test(channel.name ?? ""))
...
async function exportDescriptions(rpc, identity) {
  const outputPath = resolve(process.cwd(), flagValues("--output")[0] ?? "work/discord-descriptions.json");
  ...
  const threadIds = uniqueStrings([
    ...
    ...(hasFlag("--from-sidebar") ? await readInventorySidebarModChannels() : []),
  ]);
  if (threadIds.length === 0) {
    throw new Error("Provide --channel <channel-id>, DISCORD_RPC_CHANNEL_IDS, --from-inventory, or --from-sidebar.");
  }
  ...
  for (const threadId of threadIds) {
    try {
      ...
      unavailableSources.push({ channelId: threadId, error: error.message });
      console.warn(`Skipped unavailable Discord source ${threadId}: ${error.message}`);
    }
  }
  ...
  await writeJson(outputPath, exportDocument);
  console.log(`Wrote ${threads.length} threads and ${unavailableSources.length} unavailable sources to ${outputPath}. Review it before any CMS import.`);
}
